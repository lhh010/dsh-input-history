...
`v0.1.13`（默认） | `dsh-v0.1.2-alpha.1`~`alpha.5`、`rc.1`、`0.1.3-alpha.1`~`0.1.5-rc.1` | 声明支持 0.1.5-rc.1（npm 已发布，钉版本实机验证；rc.1 为 0.1.5 系列首个候选版本，client 插件面零代码差异；typecheck/build/18 单测全绿，启动清单确认加载） |
- 切换会话后浏览状态复位，不会跨会话续接