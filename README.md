...
dsh plugin --profile web add '@dsh-external/dsh-input-history@github:lhh010/dsh-input-history#v0.1.12'
...
> 1. 执行 `dsh plugin --profile web add '@dsh-external/dsh-input-history@github:lhh010/dsh-input-history#v0.1.12'`（首次可能被 pnpm 11 拦截 node-pty 构建脚本而失败）
...
`v0.1.13`（默认） | `dsh-v0.1.2-alpha.1`~`alpha.5`、`rc.1`、`0.1.3-alpha.1`~`0.1.5-rc.2` | 声明支持 0.1.5-rc.1~rc.2（npm 已发布，钉版本实机验证；rc.1 为 0.1.5 系列首个候选版本，client 插件面零代码差异；typecheck/build/18 单测全绿，启动清单确认加载；rc.2 兼容性复验通过：rc.2 实机宿主（tag fb2c4b9e）加载确认，无需代码改动） |
- 切换会话后浏览状态复位，不会跨会话续接