...
dsh plugin --profile web add '@dsh-external/dsh-input-history@github:lhh010/dsh-input-history#v0.1.14'
...
> 1. 执行 `dsh plugin --profile web add '@dsh-external/dsh-input-history@github:lhh010/dsh-input-history#v0.1.14'`（首次可能被 pnpm 11 拦截 node-pty 构建脚本而失败）
...
声明支持 dsh-v0.1.6-alpha.1（npm 已发布，钉版本实机验证；client 插件面零代码差异，typecheck/18 单测全绿，实机加载正常）。安装命令统一更新为 `#v0.1.14`。
